...
class categoryController {
    ...
    add_category = async (req, res) => {
        ...
        form.parse(req, async (err, fields, files) => {
            if (err) {
                responseReturn(res, 404, { error: 'Something went worng' })
            } else {
                let { name } = fields
                let { image } = files
                console.log(image)
                ...
                try {
                    const result = await cloudinary.uploader.upload(image[0].filepath, { folder: 'categorys' })
                    if (result) {
                        ...
                        responseReturn(res, 201, { category, message: 'Category Successfully Added' })
                    } else {
