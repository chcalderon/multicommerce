...
const SearchProducts = () => {
    ...
    const {searchObj,products,categorys,priceRange,latest_product,totalProduct,parPage} = useSelector(state => state.home)
    
    // const category = searchParams.get('categories')
    // const searchValue = searchParams.get('value')
    const category = searchObj.categories
    const searchValue = searchObj.textSearch
    ...
    useEffect(() => { 
        setState({
            rangeValues: [priceRange.low, priceRange.high]
        })
    },[priceRange])

    const [filter, setFilter] = useState(true) 

    const [state, setState] = useState({rangeValues: [priceRange.low, priceRange.high]})
    ...
    useEffect(() => { 
        dispatch(
            query_products({
                low: state.rangeValues[0] || '',
                high: state.rangeValues[1] || '',
                category,
                rating,
                sortPrice,
                pageNumber,
    ...
    },[state.rangeValues[0],state.rangeValues[1],category,rating,sortPrice,searchValue,pageNumber])
        ...
        <div className='py-2 flex flex-col gap-5'>
            <h2 className='text-3xl font-bold mb-3 text-slate-600'>Price</h2>
             ...
             <Range
                ...
                onChange={(rangeValues) => setState({rangeValues})}
                renderTrack={({props,children}) => {
                    const { key,...newprops} = props
                    return (
                    <div {...newprops} key={key} className='w-full h-[6px] bg-slate-200 rounded-full cursor-pointer'>
                        {children}
                    </div>
                )}}
                renderThumb={({ props }) => {
             ...
             />  
         <div>
         <span className='text-slate-800 font-bold text-lg'>${Math.floor(state.rangeValues[0])} - ${Math.floor(state.rangeValues[1])}</span>  
           </div>
         </div>
